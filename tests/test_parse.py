...
def test_short_rows_skipped():
    rows = [[], [], ["date", "a1", "a2", "b1", "b2"]]  # only 5 cols, need 7
    assert parse_matches(rows) == []


def test_single_match():
    rows = [[], [], ["15/01/2026", "Anders", "Bo", "Carsten", "Dennis", "6", "3", "6", "0"]]
...
def test_team_b_wins():
    rows = [[], [], ["01/01/2026", "A1", "A2", "B1", "B2", "3", "6", "0", "6"]]
    result = parse_matches(rows)
...
def test_draw():
    rows = [[], [], ["01/01/2026", "A1", "A2", "B1", "B2", "6", "3", "3", "6"]]
    result = parse_matches(rows)
...
def test_tiebreak_set():
    rows = [[], [], ["01/01/2026", "A1", "A2", "B1", "B2", "7", "5"]]
    result = parse_matches(rows)
...
def test_three_set_match():
    rows = [[], [], ["01/01/2026", "A1", "A2", "B1", "B2", "6", "4", "3", "6", "6", "2"]]
    result = parse_matches(rows)
    m = result[0]
    assert len(m["sets"]) == 3
...
def test_date_slash_format():
    rows = [[], [], ["15/01/2026", "A1", "A2", "B1", "B2", "6", "3", "6", "0"]]
...
def test_player_names_stripped():
    rows = [[], [], ["01/01/2026", " Anders ", " Bo ", " Carsten ", " Dennis ", "6", "3", "6", "0"]]
...
def test_egg_set():
    ...
    rows = [[], [], ["01/01/2026", "A1", "A2", "B1", "B2", "6", "0", "6", "0"]]
    m = parse_matches(rows)[0]
...
def test_unfinished_set():
    ...
    rows = [[], [], ["01/01/2026", "A1", "A2", "B1", "B2", "6", "4", "4", "6", "4", "3"]]
    m = parse_matches(rows)[0]
...
def test_multiple_matches():
    rows = [
        [], [],
        ["01/01/2026", "A1", "A2", "B1", "B2", "6", "3", "6", "0"],
        ["02/01/2026", "C1", "C2", "D1", "D2", "3", "6", "6", "3", "6", "4"],
    ]
    result = parse_matches(rows)
    assert len(result) == 2
